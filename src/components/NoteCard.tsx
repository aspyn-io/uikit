...
const NoteCard: React.FC<NoteCardProps> = ({ tags, title, content, author, time, className, avatar }) => {
  return (
    <Card className={`bg-white dark:bg-gray-800 ${className}`}>
      ...
      <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 line-clamp-2">{content}</p>
      <div className="flex justify-between items-center mt-3 text-sm text-gray-500 dark:text-gray-400">
        <div className="flex items-center">
          {avatar && <img src={avatar} alt={author} className="w-6 h-6 rounded-full mr-2" />}
