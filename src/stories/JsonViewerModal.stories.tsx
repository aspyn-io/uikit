...
const meta: Meta<typeof JsonViewerModal> = {
  ...
  argTypes: {
    ...
    invalidDataMessage: {
      control: "text",
      description:
        "Custom message to display when JSON data cannot be stringified",
      defaultValue:
        "Unable to display data: Invalid or circular JSON structure",
    },
    allowCopy: {
      control: "boolean",
...
const JsonViewerModalTemplate: React.FC<{ args: any }> = ({ args }) => {
  ...
  return (
    <div className="p-4">
      ...
      <JsonViewerModal {...args} show={show} onClose={() => setShow(false)} />
    </div>
  );
};
